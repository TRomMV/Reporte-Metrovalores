...
sector_financiero = [
    ...
    'BOLSA DE VALORES DE GUAYAQUIL', 'BOLSA DE VALORES DE QUITO', 'BANCO DEL AUSTRO'
]
sector_real = [
    'CORPORACION FAVORITA C.A.', 'CERVECERIA NACIONAL CN S A',
    'HOLCIM ECUADOR S.A.', 'SAN CARLOS SOC. AGR. IND.', 'CONCLINA C A CIA CONJU CLINICO NACIONAL', 'CONTINENTAL TIRE ANDINA S A', 'INVERSANCARLOS', 'INDUSTRIAS ALES', 'BRIKAPITAL SA', 'CRIDESA'
]
...
def obtener_precio_cierre_2025(grupo):
    """Obtener el precio de cierre del último día hábil de 2025."""
    cierre_2025 = grupo[grupo['FECHA'].dt.year == 2025].sort_values(by='FECHA').iloc[-1]
    return pd.Series([cierre_2025['FECHA'], cierre_2025['PRECIO']], index=['FECHA', 'PRECIO_CIERRE_2025'])
...
def actualizar_variacion_semanal():
    ...
    precios_cierre_2025 = df.groupby('EMISOR').apply(obtener_precio_cierre_2025, include_groups=False).reset_index()
    precios_cierre_2025.columns = ['EMISOR', 'FECHA_CIERRE_2025', 'PRECIO_CIERRE_2025']
    precios_cierre_2025 = precios_cierre_2025.set_index('EMISOR')
    precios_cierre_2025['FECHA_CIERRE_2025'] = pd.to_datetime(precios_cierre_2025['FECHA_CIERRE_2025']).dt.strftime('%Y-%m-%d')

    datos_renta_variable = precios_ultimos.join(precios_semana_anterior, how='left').join(precios_cierre_2025, how='left')
    ...
    datos_renta_variable['PRECIO_CIERRE_2025'] = datos_renta_variable['PRECIO_CIERRE_2025'].apply(lambda x: f"${x:,.2f}")
